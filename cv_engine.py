...
def find_event_timestamp_with_ai(
    ...
    if not effective_key:
        load_dotenv()
        effective_key = os.getenv("GEMINI_API_KEY", "").strip() or os.getenv("GOOGLE_API_KEY", "").strip()
...
Identify the exact time in seconds when the wicket is broken (when bails first lift, separate from stumps, or Zing bails light up).
